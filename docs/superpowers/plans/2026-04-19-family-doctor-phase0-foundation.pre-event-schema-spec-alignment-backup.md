...
```json
{
  ...
  "required_runtime_keys": ["dedupe_scope"],
  "optional_runtime_keys": ["related_runtime_id", "review_item_id"],
  ...
  "field_shapes": {
    ...
    "target": {
      "type": "object",
      "required_keys": ["member_id", "member_hint", "match_confidence"]
    },
    "payload": {
      "type": "object",
      "required_keys": ["text", "attachments", "source_refs"]
    },
    "context": {
      "type": "object",
  ...
  "attachment_shape": {
    "type": "object",
    "required_keys": ["attachment_id", "kind", "source_path"]
  }
}
```
...
TEMPLATE_MARKERS = {
    ...
    "output-template.md": ["type: output", "output_kind:", "## 结论摘要", "## 证据来源", "## 待核实项"],
    "reminder-template.md": ["type: reminder", "reminder_kind:", "## 提醒内容", "## 触发背景", "## 确认状态"],
}
...
def validate_event_schema(schema: dict) -> list[str]:
    ...
    if set(schema.get("required_runtime_keys", [])) != {"dedupe_scope"}:
        errors.append("event-schema.json has incorrect required_runtime_keys")
    if set(schema.get("optional_runtime_keys", [])) != {"related_runtime_id", "review_item_id"}:
        errors.append("event-schema.json has incorrect optional_runtime_keys")
...
def validate_sample_event(schema: dict, sample: dict) -> list[str]:
    ...
    for key in sample.get("runtime", {}):
        if key not in set(schema["required_runtime_keys"]) | set(schema.get("optional_runtime_keys", [])):
