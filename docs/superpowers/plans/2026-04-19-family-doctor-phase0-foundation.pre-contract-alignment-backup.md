...
```json
{
  ...
  "required_target_keys": ["member_id", "member_hint", "match_confidence"],
  "required_runtime_keys": ["related_runtime_id", "review_item_id", "dedupe_scope"],
...
```json
{
  ...
  "entities": {
    "ingest_job": {
      ...
      "required_fields": ["job_id", "status", "planned_writes", "completed_writes", "log_ref_field"],
      "allowed_transitions": {
      ...
      "log_ref_field": "job_id"
    },
    "report_job": {
      "primary_key": "job_id",
      "statuses": ["created", "processing", "pending_review", "committed", "failed", "aborted"],
      "required_fields": ["job_id", "status", "report_kind", "source_refs", "log_ref_field"],
      "allowed_transitions": {
        "created": ["processing", "aborted"],
        "processing": ["pending_review", "committed", "failed"],
        "pending_review": ["processing", "aborted"],
    ...
    "reminder_instance": {
      ...
      "required_fields": ["runtime_id", "member_id", "item_id", "status", "scheduled_for", "log_ref_field"],
      "allowed_transitions": {
      ...
      "log_ref_field": "runtime_id"
    },
    "review_item": {
      "primary_key": "review_item_id",
      "statuses": ["open", "resolved", "dismissed"],
      "required_fields": ["review_item_id", "reason", "status", "source_refs", "log_ref_field"],
      "allowed_transitions": {
      ...
      "log_ref_field": "review_item_id"
    },
    "dedupe_record": {
      "primary_key": "dedupe_key",
      "statuses": ["active", "archived"],
      "required_fields": ["dedupe_key", "scope", "first_seen_at", "result", "log_ref_field"],
      "allowed_transitions": {
        "active": ["archived"],
        "archived": []
      },
      "retention": "retain active for 30d; archived for audit as needed",
      "log_ref_field": "dedupe_key"
    }
  }
}
```
...
TEMPLATE_MARKERS = {
    ...
    "medication-template.md": ["type: medication", "## 适应证", "## 漏服规则"],
    "condition-template.md": ["type: condition", "## 涉及成员", "## 关键证据"],
    "trend-template.md": ["type: trend", "## 关键指标表", "## 趋势判断"],
    "plan-template.md": ["type: plan", "## 当前目标", "## 待复查事项"],
    "output-template.md": ["type: output", "## 结论摘要", "## 关联页面"],
}
...
def validate_event_schema(schema: dict) -> list[str]:
    ...
    if set(schema.get("required_target_keys", [])) != {"member_id", "member_hint", "match_confidence"}:
        errors.append("event-schema.json has incorrect required_target_keys")
    if set(schema.get("required_runtime_keys", [])) != {"related_runtime_id", "review_item_id", "dedupe_scope"}:
        errors.append("event-schema.json has incorrect required_runtime_keys")
    return errors
...
def validate_runtime_entities(registry: dict) -> list[str]:
    ...
    for name in ["ingest_job", "report_job", "reminder_instance", "review_item", "dedupe_record"]:
        ...
        if not entity:
            ...
            continue
        for key in ["primary_key", "statuses", "required_fields", "allowed_transitions", "retention", "log_ref_field"]:
        ...
        if entity.get("log_ref_field") and entity["log_ref_field"] not in entity.get("required_fields", []):
            errors.append(f"{name} log_ref_field not listed in required_fields")
...
def validate_sample_event(schema: dict, sample: dict) -> list[str]:
    ...
    for key in schema["required_target_keys"]:
